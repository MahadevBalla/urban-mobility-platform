...
def run_nb(features_path: Path) -> None:
    ...
    # Load
    print("Loading features and ward mappings...")
    df = pd.read_parquet(features_path)
    df["time_bin_30min"] = pd.to_datetime(df["time_bin_30min"], utc=True)
    
    # Load the Stop-to-Ward mapping generated by 10_ward_aggregation.py
    stop_ward_path = Path(features_path).parent / "stop_ward_map.csv"
    ...
    # Compress 99% sparse zeros into dense demand integers
    # =====================================================================
    print("Aggregating to Ward-to-Ward level...")
    
    # Map Origin Ward
    df = df.merge(
        stop_ward.rename(columns={"stop_id": "origin_stop_id", "ward_id": "origin_ward"}), 
        on="origin_stop_id", how="inner"
    )
    # Map Destination Ward
    df = df.merge(
        stop_ward.rename(columns={"stop_id": "dest_stop_id", "ward_id": "dest_ward"}), 
        on="dest_stop_id", how="inner"
    )

    # Define how to aggregate the features when combining stops into wards
    agg_dict = {
        TARGET: "sum",                          # Total trips between the two wards
        "lag_1_trip_count": "sum",              # Sum of previous trips
        "lag_2_trip_count": "sum",
        "lag_day_trip_count": "sum",
        "lag_week_trip_count": "sum",
        "rolling_24h_mean": "sum",
        "rolling_24h_std": "mean",
        "trip_distance_km": "mean",             # Average distance
        "dist_cbd_km": "mean",
        "temperature_2m": "mean",               # Average weather
        "wind_speed_10m": "mean",
        "weather_severity": "max",              # Max severity in the ward
        "log_precip": "max",
        "precip_3h": "max",
        "is_heavy_rain": "max",
        "heat_stress": "max",
    ...
    actual_agg_dict = {k: v for k, v in agg_dict.items() if k in df.columns}

    # Perform the aggregation
    df = df.groupby(["origin_ward", "dest_ward", "time_bin_30min"]).agg(actual_agg_dict).reset_index()
    ...
    # =====================================================================
    # CITYFLO DEBUG BLOCK: THE ZERO-VARIANCE DUMMY TRAP
    # =====================================================================
