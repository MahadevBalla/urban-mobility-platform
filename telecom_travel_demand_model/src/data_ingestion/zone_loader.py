...
class ZoneLoader:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self._zones: Dict[str, dict] = (
            {}
        )  # zone_id -> {centroid, bounds, population, ...}
    ...
    def create_tac_zones(
        ...
        valid_df = valid_df[(valid_df[lat_col] != 0) & (valid_df[lon_col] != 0)].copy()
    ...
    def create_grid_zones(
        self, bounds: Tuple[float, float, float, float], cell_size_m: float = 1000
    ) -> None:
        """
    ...
    def load_from_file(self, path: Union[str, Path], zone_type: str = "custom") -> None:
        """
        ...
        for _, row in df.iterrows():
            ...
            bounds = None
            if all(
                c in row.index for c in ["min_lat", "min_lon", "max_lat", "max_lon"]
            ):
                bounds = (
                    row["min_lat"],
                    row["min_lon"],
    ...
    def get_zone_for_location(self, lat: float, lon: float) -> Optional[str]:
        """
