...
from src.utils.time_utils import is_home_time, is_work_time, is_weekday
...
class HomeWorkInference:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        self.work_min_distance = hw_config.get('work', {}).get('min_distance_from_home', 500)
        self.work_min_weekly_visits = hw_config.get('work', {}).get('min_weekly_visits', 3)
    ...
    def _infer_home(
        ...
        if not scores or max(scores.values()) == 0:
            ...
            return user_stays.loc[
                user_stays['observation_count'].idxmax(), 'stay_id'
            ]
    ...
    def _calculate_home_score_from_obs(
        ...
        weekday_night_score = (
            weekday_nights_present / weekday_nights
            if weekday_nights > 0 else 0
        )
        weekend_score = (
            weekend_days_present / weekend_days
            if weekend_days > 0 else 0
        )
        early_morning_score = (
            early_mornings_present / early_mornings
            if early_mornings > 0 else 0
        )
        ...
        score = (
            weekday_night_score * 0.6 +
            weekend_score * 0.3 +
            early_morning_score * 0.1
        )
    ...
    def _infer_work(
        ...
        if home_stay['latitude'] is not None:
            distances = non_home_stays.apply(
                lambda x: haversine_distance(
                    home_stay['latitude'], home_stay['longitude'],
                    x['latitude'], x['longitude']
                ) if x['latitude'] is not None else float('inf'),
                axis=1
            )
        ...
        obs_weeks = max(1, (
            best_stay['last_seen'] - best_stay['first_seen']
        ).days / 7)
    ...
    def get_home_work_summary(self, stay_points_df: pd.DataFrame) -> pd.DataFrame:
        ...
        for user_id in stay_points_df['user_id'].unique():
            ...
            if record['has_home'] and record['has_work']:
                if all(pd.notna([record['home_lat'], record['home_lon'],
                                record['work_lat'], record['work_lon']])):
